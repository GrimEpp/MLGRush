        ...
        if (Game.isInGame(p)) {
            ...
            section.getKeys(false).forEach(s -> {
                String ss = prepare(getConfig().getColouredConfigString("settings.scoreboard.game.lines."+s, new MapBuilder().add(
                        ...
                        "%game_colored_blue%", ChatColor.BLUE + session1.getPlayer(Color.BLUE).getName(),
                        "%game_colored_red%", ChatColor.RED + session1.getPlayer(Color.RED).getName()
                ).getMap()), p);
                if (ss.length() > 32) {
                    return;
                }
                ...
                if (ss.length() > 16) {
                    ...
                    team.setSuffix(ChatColor.RESET + ss.substring(16, ss.length()));
                } else {
                    team.setPrefix(ss);
                }
                   ...
                   if (Game.isInGame(p)) {
                       ...
                       map.forEach((i, s) -> {
                           ...
                           String real = prepare(getConfig().getColouredConfigString("settings.scoreboard.game.lines."+s, new MapBuilder().add(
                                   ...
                                   "%game_colored_blue%", ChatColor.BLUE + session.getPlayer(Color.BLUE).getName(),
                                   "%game_colored_red%", ChatColor.RED + session.getPlayer(Color.RED).getName()
                           ).getMap()), p);
                           if (real.length() > 16) {
                               team.setPrefix(real.substring(0, 16));
                               team.setSuffix(ChatColor.RESET + real.substring(16, real.length()));
                           } else {
                               team.setPrefix(real);
                           }
                       });
